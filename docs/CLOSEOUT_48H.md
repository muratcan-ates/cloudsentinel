...
**State at the time of writing (Aug 1, updated late night):** 1321 tests green,
ruff clean, CI on every push, 59 endpoints, six rooms, the agent chain and HITL
...
| **T3** | **Put the live URL everywhere** — README masthead badge, submission form, video end card, `ALLOWED_ORIGINS` in `main.py`. The live host is `cloudsentinel-y5zh.onrender.com` — note the suffix; the bare `cloudsentinel.onrender.com` belongs to an unrelated app | Mert | 20 min | **Yes** |
