...
- REST API (FastAPI, 58 endpoints) with self-hosted Swagger documentation (no CDN); a `/health` liveness ping and a `/ready` readiness probe (database, mission config and dataset) for deploy/uptime gating
...
| Deployment — Render, read-only public link | Sprint 3 | ✅ shipped — the live `/health` answers `env render · provider fake · readonly True`, the standing watch is beating and the deployed surface matches this checkout (58 endpoints), all four checked by `scripts/verify_release.sh <url>` |
