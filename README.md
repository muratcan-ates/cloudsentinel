...
├── tests/                1320 pytest cases incl. performance budgets and the
...
| **pytest** | Automated test suite (1320 tests in ~41 s on the fake provider, incl. performance budgets and the endpoint contract suites below) — **96% line coverage** over 6,329 statements in `app/` + `main.py` (`make coverage`) |
...
- **Daily Scrum**: daily communication over WhatsApp with team meetings and huddles on Slack, as in the previous two sprints; the closing meeting on August 2 covered the README, the Miro board and the demo together. Sprint 3 evidence is collected in [`ProjectManagement/Sprint3Documents/`](ProjectManagement/Sprint3Documents/).

- **Sprint board update**: the Miro Sprint 3 board and the burndown export are captured by the team at the August 2 closing meeting and land in [`ProjectManagement/Sprint3Documents/`](ProjectManagement/Sprint3Documents/) alongside the product captures below. The shape of the sprint is already visible in the repository itself: **147 commits between July 20 and August 2**, with the curve running the opposite way to Sprint 2's — where Sprint 2 finished its committed scope a week early and spent the second week pulling work forward, Sprint 3 carried a steady load to the last day, because "prove it" work (deploy, measure, capture, document) cannot be front-loaded the way feature work can.
...
- **Committed scope, as it closed** (headline items — [the backlog](docs/sprint3_backlog.md) holds the detail):
  ...
  - **Continuous integration** — ✅ landed at Sprint 2 close: [`ci.yml`](.github/workflows/ci.yml) runs ruff + the full suite on every push and PR (446 cases then, 1320 now), plus an `audit` job running bandit over our own source and pip-audit over both requirement sets. Sprint 3 added the two scheduled workflows above; browser E2E stays on the post-competition list.
