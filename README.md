...
- REST API (FastAPI, 47 endpoints) with self-hosted Swagger documentation (no CDN); a `/health` liveness ping and a `/ready` readiness probe (database, mission config and dataset) for deploy/uptime gating
...
├── tests/                453 pytest cases incl. performance budgets
...
| **pytest + httpx** | Automated test suite (453 tests, incl. performance budgets) |
...
- **Remaining scope** (headline items — the backlog holds the detail):
  ...
  - **Continuous integration** — ✅ landed at Sprint 2 close: [`ci.yml`](.github/workflows/ci.yml) runs ruff + the full suite (453 tests) on every push and PR; Sprint 3 grows it with browser E2E and a post-deploy smoke.
